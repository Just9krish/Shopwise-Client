...
export const createShopAsync = createAsyncThunk('shop/createShop', async (shopData: IShopData) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createShopAsync.rejected, (state, action) => {
        state.isShopLoading = false;
        state.shopError = action.error.message ? action.error.message : 'Something went wrong';
      })
...
export const selectShopMessage = (state: RootState) => state.shopState.shopMessage;
export const selectShopLoading = (state: RootState) => state.shopState.isShopLoading;
export const selectShopAuthenticated = (state: RootState) => state.shopState.isShopAuthenticated;
