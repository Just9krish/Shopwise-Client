...
export default function generateProductName() {
  ...
  const randomAdjective = productAdjectives[Math.floor(Math.random() * productAdjectives.length)];
  const randomNoun = productNouns[Math.floor(Math.random() * productNouns.length)];
